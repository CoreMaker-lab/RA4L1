...
void gpt0_callback(timer_callback_args_t *p_args)
{
    ...
    if ((TIMER_EVENT_CAPTURE_A == p_args->event))  // 捕获事件
    {
        bsp_io_level_t p_port_value_port_415;
        // 读取端口电平状态，如果是低电平则发生的是下降沿，高电平则是上升沿
        R_IOPORT_PinRead(&g_ioport_ctrl, BSP_IO_PORT_04_PIN_15, &p_port_value_port_415);
        ...
        if (p_port_value_port_415 == BSP_IO_LEVEL_HIGH) // 上升沿
        {
            // 记录第一次和第二次上升沿的时间戳
...
void hal_entry(void)
{
    ...
    fsp_err_t err = FSP_SUCCESS;
      ...
      err = R_GPT_PeriodSet(&g_timer1_ctrl, 3200);//频率
      assert(FSP_SUCCESS == err);
      R_BSP_SoftwareDelay (20, BSP_DELAY_UNITS_MILLISECONDS);//不加延时可能会设置不成功

      err = R_GPT_DutyCycleSet(&g_timer1_ctrl, 800, GPT_IO_PIN_GTIOCA);//占空比
      assert(FSP_SUCCESS == err);
      err = R_GPT_DutyCycleSet(&g_timer1_ctrl, 2400, GPT_IO_PIN_GTIOCB);//占空比
      assert(FSP_SUCCESS == err);
      R_BSP_SoftwareDelay (20, BSP_DELAY_UNITS_MILLISECONDS);

  //   err = R_GPT_Close(&g_timer1_ctrl);
  //   assert(FSP_SUCCESS == err);
  //   R_BSP_SoftwareDelay (20, BSP_DELAY_UNITS_MILLISECONDS);
